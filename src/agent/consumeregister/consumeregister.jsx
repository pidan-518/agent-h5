...
import './consumeregister.less'
import { CarryTokenRequest } from '../../common/util/request';
import servicePath from '../../common/util/api/apiUrl';
import globalData from '../../common/util/global-data';

import logoIc from '../../static/agentPart/ic_register_logo.png'
import bgIc from '../../static/agentPart/ic_register_bg.png'

export default class consumeregister extends Component {
  constructor(props) {
    super(props);
    this.state = {
      info:{
  ...
  getsendSmsCode(mobile){
    CarryTokenRequest(servicePath.getregisterPrimarySmsCode,{phonenumber:mobile})
    .then(res=>{
  ...
  getcheckPhonenumber(mobile){
    CarryTokenRequest(servicePath.getcheckPrimaryPhonenumber,{phonenumber:mobile})
    .then(res=>{
      console.log('发送短信验证码',res);
      if(res.data.code===0){
  ...
  getRegisterPrimaryAgent(){
    ...
    CarryTokenRequest(servicePath.getRegisterPrimaryAgent,this.state.info)
    .then(res=>{
      ...
      if(res.data.code===0){
        ...
        setTimeout(() => {
          ...
          switch (envConstants) {
            case 'pro':
              shareLink = 'https://aigangmao.com/h5/v100/#/pages/landingpage/landingpage'
              break;
            case 'pre':
              shareLink = 'http://121.42.231.22/h5/#/pages/landingpage/landingpage' 
              break;
            case 'dev':
              shareLink = 'http://192.168.0.181/h5/#/pages/landingpage/landingpage'
              break;
            default:
              break;
          }
  ...
  getCode=()=>{ // 发送验证码
    if (!this.state.info.phonenumber) {
      ...
      if (!(/^1[3456789]\d{9}$/.test(this.state.info.phonenumber))&&!(/^(5|6|8|9)\d{7}$/.test(this.state.info.phonenumber))) {
        Taro.showToast({
          title: '手机号格式错误',
          icon: 'none',
          duration: 2000
  ...
  getSubmit=()=>{
    ...
    if (!(/^1[3456789]\d{9}$/.test(phonenumber))&&!(/^(5|6|8|9)\d{7}$/.test(phonenumber))) {
      Taro.showToast({
        title: '请输入正确的手机号',
        icon: 'none',
        duration: 2000
    ...
    this.getRegisterPrimaryAgent()
  }
  config = {
    navigationBarTitleText: '消费者会员注册'
  }

  render () {
    const { info,time, } = this.state
    return (
      <View className='consumeregister'>
        {/* <Image className="bg" src={bgIc}></Image> */}
          ...
          <View className="form">
            ...
            {/* <View className="tips">战略合作伙伴-爱港猫</View> */}
